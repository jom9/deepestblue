...
Square * Square::clone(){
	...
	S->rank = this->rank;
	S->file = this->file;
	S->piece = this->piece->clone();
	S->attackedByWhite = this->attackedByWhite; // number of pieces attaking this square
	S->attackedByBlack = this->attackedByBlack;
	S->color =this->color;
...
Board::Board(){// board constructor
													...
													for(int i=0;i<8;i++){
														board[i][1].piece = new Pawn('w',i,1);// sets white pawns
													...
													board[0][0].piece = new Rook('w',0,0);// sets rooks
													board[7][0].piece = new Rook('w',7,0);
													board[0][7].piece = new Rook('b',0,7);
													board[7][7].piece = new Rook('b',7,7);
													...
													board[1][0].piece = new Knight('w',1,0);// sets rooks
													board[6][0].piece = new Knight('w',6,0);
													board[1][7].piece = new Knight('b',1,7);
													board[6][7].piece = new Knight('b',6,7);
													blackPos.push_front(make_tuple(1,7));
													blackPos.push_front(make_tuple(6,7));
													whitePos.push_front(make_tuple(1,0));
													whitePos.push_front(make_tuple(6,0));

													board[2][0].piece =  new Bishop('w',2,0);// sets bishop
													board[5][0].piece = new Bishop('w',5,0);
													board[2][7].piece = new Bishop('b',2,7);
													board[5][7].piece = new Bishop('b',5,7);
													blackPos.push_front(make_tuple(2,7));
													blackPos.push_front(make_tuple(2,7));
													whitePos.push_front(make_tuple(2,0));
													whitePos.push_front(make_tuple(5,0));

													board[3][0].piece =  new Queen('w',3,0);// sets major piece
													board[4][0].piece = new King('w',4,0);
													board[3][7].piece = new Queen('b',3,7);
													board[4][7].piece =  new King('b',4,7);
...
bool Board::updateBoard(){
	for(int i = 0; i<8; i++){
		...
		for(int i = 0; i<8; i++){
			for(int j =0 ; j<8; j++){
						//board[i][j].piece->legalMoves.clear();
	...
	if(currentPlayer=='w'){
		for(list<tuple <int,int>>::iterator it1=whitePos.begin(); it1 != whitePos.end(); ++it1){
			...
			for(list<tuple <int,int>>::iterator it2=P.legalMoves.begin(); it2 != P.legalMoves.end(); ++it2){
				...
				//cout<<get<0>(*it1)<<"\t"<<	get<1>(*it1)<<"\t"<<	 get<0>(*it2)<<"\t"<<get<1>(*it2)<<"\n";
			}
		}
	}
	else{
...
Board * Board::clone(){
	...
	for(int i=0;i<8;i++){
		for(int j=0;j<8;j++){
			B->board[i][j].piece = this->board[i][j].clone();
		}
	}
	return B;
